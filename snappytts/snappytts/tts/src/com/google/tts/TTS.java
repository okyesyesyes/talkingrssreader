...
public class TTS {
	...
	/**
	 * Called when the TTS has finished speaking by itself (ie, speaking
  ...
  private boolean disconnected = false;
	 ...
	 *            initialized successfully.
	 * @param alert
	 *            The TTSVersionAlert to be displayed
	 */
	...
	public void setOnSpeechCompletedListener(
			final SpeechCompletedListener listener) {
					...
					if (version < MIN_VER) {
						if (showInstaller) {
							if (versionAlert != null) {
								...
								new TTSVersionAlert(ctx, null, null, null)
										.show();
							}
						}
						return;
	 ...
	 * 
	 * @param earcon
	 *            The TTSEarcon that should be played
	 * @param queueMode
	 *            0 for no queue (interrupts all previous utterances), 1 for
	 *            queued
	 * @param params
  ...
  public static abstract class TTSUserCallback extends ITTSUserCallback.Stub {
	 ...
	 * on the Market. This is useful if the application expects a newer version
